 ...
 * GET /api/markets?tickers=BZ=F,GC=F,^VIX&range=5d&interval=15m
 *
 * Returns { results: { ticker, meta, chart }[] }
 *
 * Server-side cache: 2 min fresh, 10 min stale-while-revalidate.
 */
  ...
  try {
    ...
    for (let i = 0; i < timestamps.length; i++) {
      ...
      if (val != null && !isNaN(val)) {
        chart.push({ time: timestamps[i], value: val });
      }
    }

    const price = meta.regularMarketPrice ?? (chart.length > 0 ? chart[chart.length - 1].value : 0);
...
export async function GET(req: NextRequest) {
  ...
  const results = await Promise.all(
    tickers.slice(0, 20).map(t => getCached(t, range, interval)),
  );
